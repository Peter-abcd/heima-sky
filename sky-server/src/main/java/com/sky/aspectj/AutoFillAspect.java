        ...
        if (value.equals(OperationType.INSERT)) {
            for (Map.Entry<Object, Method[]> entry : methodHashMap.entrySet()) {
                ...
                for (Method method1 : entry.getValue()) {
                    if (method1.getName().equals(AutoFillConstant.SET_UPDATE_TIME) || method1.getName().equals(AutoFillConstant.SET_CREATE_TIME)) {
                        method1.invoke(key, LocalDateTime.now());
                    }
                    if (method1.getName().equals(AutoFillConstant.SET_CREATE_USER) || method1.getName().equals(AutoFillConstant.SET_UPDATE_USER)) {
